...
sudo mkdir -p /srv/aws-react-node-mysql-app/backend
echo "Backend directory is ready at /srv/aws-react-node-mysql-app/backend"
...
sudo rm -rf /srv/aws-react-node-mysql-app/backend/{*,.*} 2>/dev/null
